...
/**
 * @property \RipDB\Model\MetaModel $model
 */
class MetaController extends Controller
{
	...
	{
		parent::__construct($page, new m\MetaModel());
	}
	...
	public function performRequest(array $data = []): void
	{
		switch ($this->getPage()) {
			...
			case 'metas/new':
				break;
			case 'metas/edit':
				$meta = $this->model->getMeta($data['id']);
				if (empty($meta)) {
		...
		switch ($this->getPage()) {
			...
			case 'metas/new':
				...
				switch ($this->getPage()) {
					case 'metas/new':
						$metaID = 0;
						$result = $this->submitRequest($validated, 'usp_InsertMeta', '/metas', 'Meta Joke successfully added!', $metaID);
						break;
					case 'metas/edit':
						$result = $this->submitRequest($validated, 'usp_UpdateMeta', '/metas', 'Meta Joke successfully updated!');
						break;
				}
				break;
		}
